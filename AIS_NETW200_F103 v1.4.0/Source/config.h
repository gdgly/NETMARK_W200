#ifndef __CONFIG_H
#define __CONFIG_H
#include <stdlib.h>
#include <stdio.h>
#include "stm32f10x.h"//#include "stm32f10x_lib.h"
#include "stm32f10x_flash.h"
//#include "misc.h"

typedef unsigned long  UINT32;
typedef unsigned short UINT16;
typedef unsigned char  UINT8;

#define PLL_ON()       GPIO_SetBits(GPIOB, GPIO_Pin_12) //PLL开
#define PLL_OFF()      GPIO_ResetBits(GPIOB, GPIO_Pin_12)  //PLL关

#define GPS_ON()       GPIO_SetBits(GPIOB, GPIO_Pin_5) //GPS开
#define GPS_OFF()      GPIO_ResetBits(GPIOB, GPIO_Pin_5)  //GPS关

#define PA_ON()        GPIO_SetBits(GPIOB, GPIO_Pin_14) //功放开
#define PA_OFF()       GPIO_ResetBits(GPIOB, GPIO_Pin_14)  //功放关

#define LED_ON()       GPIO_SetBits(GPIOC, GPIO_Pin_13) //LED开
#define LED_OFF()      GPIO_ResetBits(GPIOC, GPIO_Pin_13)  //LED关

//Configures the system clocks. 系统时钟 64M； PCLK1:32M ； PCLK2：64M
// void RCC_Configuration(void);

// Configures the different GPIO ports. CAN TRX config； SPI_IO
void GPIO_Configuration(void);

//Configures the NVIC and Vector Table base address.
//void NVIC_Configuration(void);

//initial stm32
void stm32_init(void);

void SYS_CLK_UP(void);
void SYS_CLK_DOWN(void);



#endif

