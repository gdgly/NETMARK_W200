...
//u8 boatnum[16]={0x60,0x34,0x93,0x8c,0x24,0x13,0x99,0x57,0x0d,0x73,0xce,0xdc,0x31,0x00,0x00,0x00}; //船名 MINLIANYU0533-01
//u8 boatnum[16]=  {0x60,0x34,0x93,0x8c,0x24,0x13,0x99,0x57,0x0d,0x73,0xcd,0xcb,0x70,0xc4,0x00,0x00}; //船名 MINLIANYU0533\-01
//u8 boatnum[16]=  {0x60,0x34,0x93,0x8c,0x24,0x13,0x99,0x57,0x0d,0x73,0xcd,0xfc,0x31,0x00,0x00,0x00}; //船名 MINLIANYU0533_01
//u8 boatnum[16]=  {0x60,0x34,0x93,0x8c,0x24,0x13,0x99,0x57,0x0d,0x73,0xce,0x0b,0x60,0xc3,0x10,0x00}; //船名 MINLIANYU0533 - 01
...
//ais_status clk_convert_flag=on;//主频时钟切换标志
u8 flag_channel=0;		 //信道选择标志位,初始化为信道1
									
/*********************gps.c*************************/									
ais_status flag_gps_valid=off;  //初值一定为off,gps位置信息有效数据标志，复位后gps初值无效。
ais_status gps_off_flag=off;   //关闭GPS使能，为on时才允许关闭GPS。
u8 gps_off_cnt=0; //记录gps更新位置信息次数，更新N次后关闭gps
GPS_INFO GPS; //gps信息结构体

/*********************exti.c*************************/	
ais_status reset_flag=off; //复位标志，上电默认为on.根据该标志判断是否频偏矫正。
ais_status flag_fre_check=off; //频偏矫正任务标志，初值必须复设为off，上电默认不分配任务。
ais_status freq_pn_flag=on; //晶振频偏正负标志，on表示频偏为正，off标志频偏为负。
...
ais_status key_off_flag=off; //钥匙关闭网位仪标志，默认为off
u8 key_off_cnt=0;//关闭网位仪计数，只响应一次钥匙关闭
ais_status key_on_flag=on; //钥匙关闭网位仪标志，默认为on
...
u8 wireless_en=0;// 无线读写码使能，默认不能读写
u8 wireless_cnt=0;//检测到充电状态次数，只有先检测到充电状态才可能使能无线读写。

u8 rxbuf1[4] = {0x06,0x9F,0x6B,0xC7}; //temp////////////////////
/*********************spi_nrf.c*************************/
//u8 rxflag; //发送标志,防止发送时进入中断
//u8 status;
...
int main(void)
{
// 	GPIO_SetBits(GPIOB, GPIO_Pin_12); //PB12   总准备时间：8.5ms
...
// 	
 ...
 SKY72310_Init();
	...
	reset_flag=off;
	charging_flag=off;  wireless_en=0;
	//RTC_Init();
//interval_s = 5;
	while (1)
  {
