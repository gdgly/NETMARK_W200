...
//   		while (!(USART1->SR & USART_FLAG_TXE)) ;
		...
		if (status == 'A')  //如果数据有效，则分析
		{
			...
			if(t_convert==3)
			{
				txbuf[0]='$'; txbuf[1]=0x1C; txbuf[2]=0x01; //GPS有效指令
				sendback(); //
			 ...
			 if (cogsel == 0)
				...
				//direction = 0; //此版强制要求航向为零
				
				//direction = Get_Double_Number_direction(&buf[GetComma( 8, buf)]);
				
				//夜间检测
				gpstime = Get_Double_Number_time(&buf[GetComma( 1, buf)]); 
				if(gpstime >= 93000 && gpstime < 220000)  //晚6-早6
				{
					txbuf[0]='$'; txbuf[1]=0x29; txbuf[2]=0x01; //开启夜晚接收
					sendback(); //
				}
				else if(gpstime >= 220000)
				{
					txbuf[0]='$'; txbuf[1]=0x29; txbuf[2]=0x00; //关闭夜晚接收
					sendback(); //
				}
